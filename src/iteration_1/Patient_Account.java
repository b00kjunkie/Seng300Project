...
//	private static final String pattern = ".*[A-Z].*";
	...
	//patient special ID; we can consider to be string if we want to use both letter and number 
	...
	// appointments variable is a string array contain name or content of the appointment. 
	...
	public static boolean checkUsername(String usrname) {
		...
		if(usrname.length() <= min_user_name || usrname.length() >= max_user_name) {
	...
	/*
	 * this method will check the password allowed under two conditions
